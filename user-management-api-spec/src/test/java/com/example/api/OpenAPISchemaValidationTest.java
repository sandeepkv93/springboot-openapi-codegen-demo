...
class OpenAPISchemaValidationTest {
  ...
  OpenAPISchemaValidationTest() throws IOException {
    ...
    try (InputStream inputStream = resource.getInputStream()) {
      SwaggerParseResult result =
          new OpenAPIParser().readContents(new String(inputStream.readAllBytes()), null, options);

      assertThat(result.getMessages()).as("OpenAPI parsing should not have any errors").isEmpty();
  ...
  void shouldHaveBasicInfo() {
    ...
    assertThat(openAPI.getInfo().getVersion()).as("API version should be set").isEqualTo("1.0.0");

    assertThat(openAPI.getServers()).as("API should have server configuration").isNotEmpty();
  }
  ...
  class UserEndpoints {
    ...
    void shouldHaveGetUsersEndpoint() {
      ...
      // Then
      assertThat(getOperation).as("/users should have GET operation").isNotNull();
    ...
    void shouldHavePostUsersEndpoint() {
      ...
      // Then
      assertThat(postOperation).as("/users should have POST operation").isNotNull();
      ...
      assertThat(postOperation.getRequestBody()).as("Should have request body").isNotNull();
  ...
  class UserSchema {
    ...
    void shouldHaveProperPropertyDefinitions() {
      ...
      assertThat(idProperty.getReadOnly()).as("ID should be read-only").isTrue();

      // Name property
      Schema<?> nameProperty = properties.get("name");
      assertThat(nameProperty.getMinLength()).as("Name should have minimum length").isEqualTo(1);
      assertThat(nameProperty.getMaxLength()).as("Name should have maximum length").isEqualTo(100);
  ...
  void shouldDefineErrorResponses() {
    ...
    assertThat(getResponses.get("500")).as("GET should have 500 error response").isNotNull();
